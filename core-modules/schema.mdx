...
// Access the element schema via `.value`, for consistency with other
// collection wrappers like `Schema.Chunk` and `Schema.HashSet`:
const ElementSchema = NumberArraySchema.value // Schema.Number
```
...
5. **Prefer makeOption over make**: Use `makeOption` when validation failure is expected
